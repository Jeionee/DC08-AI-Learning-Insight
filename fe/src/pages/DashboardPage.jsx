  ...
  useEffect(() => {
    async function fetchDailyProgress() {
      try {
        ...
        const progress = response.daily_progress[0] || {
          hours_spent: 0,
          target: 0,
        };
        ...
        setDailyProgress({
          ...
          time_spent_hours: parseFloat(progress.hours_spent),
          target_hours: parseFloat(progress.target),
          percentage:
            (parseFloat(progress.hours_spent) / parseFloat(progress.target)) *
            100,
        });
      } catch (error) {
        console.error("Gagal mengambil daily progress:", error);
      }
    }
    fetchDailyProgress();
  }, []);
  ...
  useEffect(() => {
    async function fetchQuizScores() {
      try {
        ...
        setQuizScores(response); // jangan biarkan state undefined
      }
    }
    fetchQuizScores();
  }, []);
          ...
          <div className="xl:col-span-2 flex flex-col gap-8">
            ...
            <div className="bg-white rounded-[2rem] p-8 shadow-xl shadow-slate-200/50 border border-slate-100">
              ...
              <div className="min-h-[350px]">
                <Charts weekly={weeklyProgress} quizScores={quizScores} />
              </div>
            </div>
          </div>
              ...
              <div className="bg-white rounded-3xl p-5 shadow-lg shadow-indigo-100/50 border border-slate-100 flex flex-col items-center justify-center text-center group hover:-translate-y-1 transition-transform duration-300">
                ...
                <div className="flex items-baseline gap-1">
                  <span className="text-3xl font-extrabold text-slate-800">
                    {dailyProgress.time_spent_hours.toFixed(1)}
                  </span>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Target: {dailyProgress.target_hours} Jam
                </p>
              </div>
