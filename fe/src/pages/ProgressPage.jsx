          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
              ...
              <p className="text-slate-500 text-lg mt-2 max-w-xl leading-relaxed">
                Pantau terus perkembangan skillmu. Setiap langkah kecil
                membawamu lebih dekat ke tujuan! 🚀
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm text-slate-400 font-medium">
              {dummyModules.length} Course Active
            </span>
          </div>
...
function StatCard({ icon: Icon, title, value, subtitle, color }) {
  ...
  return (
    <div
      className={`relative shadow-lg overflow-hidden p-6 rounded-[2rem] border ${theme.border} ${theme.bg} hover:-translate-y-1 transition-transform duration-300 group`}
    >
      <Icon
        className={`absolute -right-4 -bottom-4 w-32 h-32 opacity-10 ${theme.text} rotate-12`}
      />

      <div className="relative z-10">
        <div
          className={`w-12 h-12 rounded-xl ${theme.iconBg} shadow-sm flex items-center justify-center mb-4 ${theme.text}`}
        >
          <Icon size={24} />
        </div>
        <div>
          <p
            className={`${theme.text} text-xs font-bold uppercase tracking-wider mb-1 opacity-80`}
          >
            {title}
          </p>
...
function ModuleCard({ module }) {
  ...
  return (
    <div
      className={`bg-white rounded-[2rem] border border-slate-100 transition-all duration-300 group ${
        isOpen
        ...
        <div className="p-6 pl-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {module.assessments.map((a, idx) => (
              <div
                key={idx}
                className="bg-white p-4 rounded-2xl border border-slate-100 flex items-center justify-between hover:border-indigo-200 transition-colors shadow-sm"
              >
                <div>
