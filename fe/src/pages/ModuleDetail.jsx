...
import {
  ArrowLeft, BookOpen, FileText, PlayCircle, Trophy, CheckCircle, Lock,
  ChevronLeft, ChevronRight, UploadCloud, Award, Star, Download, Share2, Calendar, Hash, Home
} from "lucide-react";
...
const dummyModules = [
  {
    ...
    lessons: [
      ...
      { id: 3, title: "Quiz 1: HTML Basics", type: "quiz", duration: "10 soal", completed: false, content: { type: "quiz" } },
      { id: 4, title: "Final Project: Personal Website", type: "project", duration: "2 jam", completed: false, content: { type: "project" } },
    ],
  },
  // ... modul lain
];
...
export default function ModuleDetail() {
  ...
  if (!module) return <div className="min-h-screen flex items-center justify-center bg-slate-50">Modul tidak ditemukan.</div>;
      ...
      <div className="min-h-screen bg-[#F8FAFC] font-sans selection:bg-indigo-100 selection:text-indigo-900 relative flex flex-col md:flex-row">
        ...
        <aside className="w-full md:w-80 lg:w-96 bg-white border-r border-slate-200 h-screen overflow-y-auto sticky top-0 z-20 flex flex-col shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
          <div className="p-6 border-b border-slate-100 bg-slate-50/50">
            ...
            <h2 className="font-extrabold text-xl text-slate-800 leading-snug">{module.title}</h2>
          </div>
          ...
          <div className="flex-1 p-4 space-y-2 overflow-y-auto custom-scrollbar">
            {module.lessons.map((lesson, idx) => {
              ...
              const isLocked = !lesson.completed && idx > 0 && !module.lessons[idx-1].completed; 
              ...
              return (
                ...
                >
                  <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 text-sm transition-colors ${
                    isActive ? "bg-white/20 text-white" : lesson.completed ? "bg-emerald-100 text-emerald-600" : "bg-slate-100 text-slate-400"
                  }`}>
                    {lesson.completed ? <CheckCircle size={16} /> : isLocked ? <Lock size={14} /> : isActive ? <PlayCircle size={16} fill="currentColor" /> : <BookOpen size={14} />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-semibold truncate ${isActive ? "text-white" : "text-slate-700"}`}>{lesson.title}</p>
                    <div className={`flex items-center gap-2 text-[10px] mt-0.5 ${isActive ? "text-indigo-200" : "text-slate-400"}`}>
        ...
        <main className="flex-1 flex flex-col h-screen overflow-y-auto bg-[#F8FAFC]">
          ...
          <header className="h-20 border-b border-slate-200 flex items-center justify-between px-8 bg-white/80 backdrop-blur-md sticky top-0 z-10">
            <h3 className="font-bold text-slate-800 truncate mr-4 text-lg">{activeLesson.title}</h3>
          ...
          <div className="flex-1 p-6 lg:p-10 max-w-5xl mx-auto w-full">
            <div className="bg-white rounded-[2.5rem] p-8 lg:p-12 shadow-xl shadow-slate-200/50 border border-slate-100 min-h-[600px] flex flex-col relative overflow-hidden">
              ...
              <div className="flex-1">
                {activeLesson.type === "video" ? (
                  <div className="w-full aspect-video bg-slate-900 rounded-3xl flex items-center justify-center text-white shadow-2xl relative overflow-hidden group cursor-pointer ring-8 ring-slate-50">
                    ...
                    <PlayCircle size={80} className="relative z-10 opacity-90 group-hover:scale-110 transition-transform duration-300 drop-shadow-lg" />
                    <p className="absolute bottom-8 left-8 text-white font-bold z-10 text-lg">Putar Video Pembelajaran</p>
                  </div>
                ) : activeLesson.type === "article" ? (
                  <div className="prose prose-slate prose-lg max-w-none text-slate-600">
                    <div dangerouslySetInnerHTML={{ __html: activeLesson.content.body }} />
                  </div>
                ) : (
                  <div className="flex flex-col items-center justify-center h-96 text-center border-2 border-dashed border-slate-200 rounded-[2rem] bg-slate-50/30">
                    <div className="p-6 bg-white rounded-3xl shadow-sm mb-6 text-amber-500 ring-4 ring-amber-50">
                      {activeLesson.type === 'project' ? <UploadCloud size={48} /> : <Trophy size={48} />}
                    </div>
                    <h3 className="text-2xl font-bold text-slate-800 mb-2">{activeLesson.type === 'project' ? 'Final Project' : 'Quiz Time!'}</h3>
                    ...
                    <button onClick={handleNext} className="px-10 py-4 bg-indigo-600 text-white rounded-2xl font-bold hover:bg-indigo-700 transition shadow-xl shadow-indigo-200 hover:-translate-y-1">
                      {activeLesson.type === 'project' ? 'Mulai Submit Project' : 'Mulai Quiz'}
                    </button>
                  </div>
                )}
              </div>
              ...
              {/* Navigation Footer */}
              {activeLesson.type !== 'project' && activeLesson.type !== 'quiz' && (
                <div className="mt-12 pt-8 border-t border-slate-100 flex justify-between items-center">
                  ...
                  >
                    {isLastLesson ? "Selesaikan Modul" : "Materi Selanjutnya"} <ChevronRight size={20} />
                  </button>
                </div>
              )}
            </div>
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] flex items-center justify-center p-4 font-sans relative">
        ...
        <div className="bg-white max-w-xl w-full rounded-[2.5rem] shadow-2xl shadow-slate-200/50 p-10 lg:p-12 border border-slate-100 relative overflow-hidden">
          ...
          <button onClick={() => setView('learning')} className="absolute top-6 left-6 p-2 rounded-full hover:bg-slate-100 text-slate-400 hover:text-slate-700 transition">
            <ArrowLeft size={24} />
          </button>
          ...
          <h2 className="text-3xl font-extrabold text-center text-slate-800 mb-3 leading-tight">Submit Final Project</h2>
          <p className="text-center text-slate-500 mb-10 text-lg leading-relaxed">
            Selamat! Kamu telah menyelesaikan semua materi. Kirimkan link proyekmu untuk direview.
          </p>

          <form onSubmit={handleSubmitProject} className="space-y-6">
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-2 ml-1">Link Repository / Deployment</label>
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] flex items-center justify-center p-6 font-sans relative overflow-hidden">
        ...
        <div className="max-w-4xl w-full text-center relative z-10">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold text-slate-800 mb-4 tracking-tight leading-tight">Selamat! Kamu Lulus! 🎉</h1>
          <p className="text-slate-500 mb-12 text-xl leading-relaxed max-w-2xl mx-auto">
            Kamu berhasil menyelesaikan modul <strong>{module.title}</strong>. Berikut adalah sertifikat pencapaianmu.
          </p>
            ...
            <div className="bg-white rounded-[2rem] p-10 md:p-16 border-[6px] border-double border-slate-50 relative overflow-hidden">
              ...
              <div className="flex justify-center mb-8 relative">
                <div className="flex gap-1.5">
                  {[...Array(5)].map((_, i) => <Star key={i} size={24} className="fill-amber-400 text-amber-400 drop-shadow-sm" />)}
                </div>
              </div>
              
              <h2 className="text-3xl md:text-4xl font-serif font-bold text-slate-800 mb-3 tracking-wide">SERTIFIKAT KELULUSAN</h2>
              <p className="text-slate-500 font-medium mb-10 uppercase tracking-widest text-sm">Diberikan Dengan Bangga Kepada</p>
              ...
              <p className="text-slate-600 text-lg mb-2 font-medium leading-relaxed">
                Telah sukses menyelesaikan semua materi dan proyek pada modul pembelajaran:
              </p>
              <h4 className="text-2xl font-bold text-slate-800 mb-12">{module.title}</h4>
              ...
              <div className="flex flex-col md:flex-row justify-between items-end border-t-2 border-slate-100 pt-8 mt-auto gap-6 md:gap-0">
                <div className="text-left">
                  <div className="flex items-center gap-2 text-slate-500 text-sm font-bold mb-1">
                    <Calendar size={16} className="text-indigo-400" /> Tanggal Lulus
                  </div>
                ...
                <div className="hidden md:block text-center opacity-80">
                   ...
                   <p className="text-slate-500 text-sm font-bold">LeanSmart Academy</p>
                </div>
                ...
                <div className="text-left md:text-right">
                  ...
                  <p className="font-mono text-slate-800 font-bold text-lg tracking-wider">{certId}</p>
                </div>
              </div>
